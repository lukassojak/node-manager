...
- Added reusable sidebar components:
  ...
  - `SidebarSection`
  - `SidebarItem`
- Added initial `MainDashboardPage` with placeholder content
...
### Changed
- Refactored page header into reusable `GlassPageHeader` component with polished reactive glass style
- Introduced unified card architecture:
  - `NodeCardStructural`
  - `ZoneCardStructural`
- Removed legacy inline clickable tile patterns
- Replaced scale-based hover effects with subtle structural elevation and border highlight
- Introduced consistent solid surface style for dashboard and detail cards
- Unified visual language across Dashboard and Node detail views
- Separated layout components from UI interaction components (buttons, cards)
